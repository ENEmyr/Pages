...
RoleId = NewType('RoleId', PositiveInt)
...
class UserInfo(BaseModel):
    ...
    role_id: RoleId = Field(default=None, description='If not defined role will be user(555) by default.') # need to replace with role model later
    gender: bool = Field(default=True, description='If not defined role will be male(true) by default.') # male = 1, female = 0
    rank: PositiveInt = Field(default=0, exclusiveMaximum=int('9'*11),description='Rank of user')
...
class UserRegis(UserSignin):
    ...
    role_id: RoleId = Field(1, description='If not defined role will be user(false) by default.') 
